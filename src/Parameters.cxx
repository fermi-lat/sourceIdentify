...
#include "sourceIdentify/Log.h"              // for parameter dumping/errors
...
Status Parameters::load(st_app::AppParGroup &pars, Status status) {
    ...
    do {
      ...
      std::string p_probMethod = pars["probMethod"];
      ...
      m_probMethod             = p_probMethod;
      probThres                = pars["probThres"];
      maxNumCtp                = pars["maxNumCtp"];
...
#else
Status Parameters::load(Status status) {
    ...
    do {
      ...
      sprintf(srcCatName, "../data/3EG_all.tsv");
      sprintf(cptCatName, "../data/1rxs_50.tsv");
      sprintf(outCatName, "output.fits");
      sprintf(srcCatQty,  "3EG,RAJ2000,DEJ2000,theta95");
      sprintf(cptCatQty,  "*");
      sprintf(probMethod,  "POSITION");
      numOutCatQty = 0;
      outCatQty    = NULL;
      probThres    = 0.05;
      maxNumCtp    = 100;
...
Status Parameters::dump(Status status) {
    ...
    do {
      ...
      Log(Log_1, " Probability method ...............: %s", 
          m_probMethod.c_str());
      Log(Log_1, " Probability threshold ............: %e", probThres);
      Log(Log_1, " Maximum number of counterparts  ..: %d", maxNumCtp);
      ...
      Log(Log_1, " Clobber ..........................: %d", clobber);
      Log(Log_1, " Verbose level ....................: %d", verbose);
